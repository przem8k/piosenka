...
class InvitationTest(TestCase):
    ...
    def test_invite_view_post_valid(self):
        ...
        self.assertEqual(0, len(Invitation.objects.filter(email_address=email_address)))
        ...
        self.assertEqual(1, len(Invitation.objects.filter(email_address=email_address)))
    ...
    def test_join_view_post_empty(self):
        email_address = "alice@example.com"
        invitation = Invitation.create_for_testing(email_address, testing.create_user())
    ...
    def test_join_view_post_valid(self):
        ...
        self.assertEqual("everyone", new_user.groups.values_list("name", flat=True)[0])

        another_client = testing.get_client()
        response = another_client.post(invitation.get_invitation_url(), self._JOIN_DATA)
        self.assertEqual(404, response.status_code)
