...
def parse_lyrics(raw_lyrics):
    """
        returns parsed lyrics as a list of paragraphs, each paragraph represented as a 
        lists of tuples representing lines, each line as (text, chords, is_indented, are_chords_extra)
    """
    result = []
    ...
    for raw_line in raw_lyrics.split('\n'):
        ...
        if line.startswith('#'):
            ...
            if mode == LyricsParserMode.Recording:
                if are_chords_extra:
                    raise SyntaxError, "Paragraph tagged for reference cannot contain repeated chords / chords in {} brackets"
