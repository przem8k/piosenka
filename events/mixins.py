...
from events.models import Event, Performer, Venue
...
class EventMenuMixin(object):
    ...
    def get_year_span(self):
        ...
        earliest = (Event.objects.first().datetime.year
                    if Event.objects.first() else cur)
    ...
    def get_context_data(self, **kwargs):
        ...
        context['active_performers'] = Performer.objects.all() \
            .annotate(event_count=Count('entityperformance')) \
            .order_by('-event_count')[:EventMenuMixin.PERFORMER_COUNT]
        return context
